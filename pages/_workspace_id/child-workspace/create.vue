...
export default {
  ...
  middleware: ['authCheck', 'can-create-childWorkspace'],
  data() {
    return {
      loading: false,
      form: {
  ...
  methods: {
    async handleSubmit() {
      ...
      try {
        ...
        formData.append('workspace_id', this.form.workspace_id)
        ...
        const { data } = await this.$axios.$post('add-new-workspace', formData)
        await this.$auth.fetchUser()
        data.id && this.$setCurrentWorkspace(data.id)
        this.loading = false
        return this.$router.replace({
          name: 'workspace_id-child-workspace-choose-modules',
          params: { workspace_id: data.id },
        })
      } catch (e) {
        this.loading = false
