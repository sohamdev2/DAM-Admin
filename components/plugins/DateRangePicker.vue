...
export default {
  ...
  props: {
    ...
    limitedDates: { type: [Boolean], default: false },
  },
  data() {
    return {
      fp: null,
  ...
  mounted() {
    ...
    if (this.limitedDates) {
      this.fp.set('maxDate', new Date(this.startDate).fp_incr(90))
      this.fp.set('minDate', new Date(this.startDate).fp_incr(-90))
    }
  ...
  methods: {
    ...
    onChange(selectedDates, dateStr, instance) {
      if (this.limitedDates) {
        this.fp.set('maxDate', new Date(selectedDates[0]).fp_incr(90))
        this.fp.set('minDate', new Date(selectedDates[0]).fp_incr(-90))
      }
