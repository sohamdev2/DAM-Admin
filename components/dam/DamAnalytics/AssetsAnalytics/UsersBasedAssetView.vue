<template>
  <div class="tableDetail">
    <div
      class="common-box"
      :class="{ 'bg-gray': topUserBasedViewAssetsList.length }"
    >
      <div class="table-list-view table-list-scrolling">
        <ul v-if="topUserBasedViewAssetsList.length" class="thead">
          <li>
            <div class="user-name sorting flex50">
              <span>User Name</span>
            </div>
        ...
        <div class="customscrollbar no_footer">
          ...
          <template v-else>
            <ContentLoader
              v-if="listLoading && !topUserBasedViewAssetsList.length"
              :speed="1"
              :animate="true"
              :width="300"
              :height="140"
            ...
            <ul v-if="topUserBasedViewAssetsList.length" class="tbody">
              <li v-for="file in topUserBasedViewAssetsList" :key="file.id">
                <div class="user-name tb-column flex50">
                ...
                <div class="total-viewed tb-column flex25">
                  <div class="top-column">
...
export default {
  ...
  computed: {
    ...
    files: {
      get() {
        return this.topUserBasedViewAssetsList
      },
      set(value) {
        this.topUserBasedViewAssetsList = value
      },
    },
  },
  watch: {
    startDate: {
      deep: true,
      handler() {
        this.getTopUserBaseViewedAssets()
      },
    },
  ...
  methods: {
    ...
    async getTopUserBaseViewedAssets() {
      if (!this.userAnalyticsAllowed) return
      this.listLoading = true
      await this.$axios
        .$post('digital-assets/analytics/top-user-based-on-view', {
          from_date: this.startDate,
          to_date: this.endDate,
        })
        .then(({ data }) => {
          this.topUserBasedViewAssetsList = data
