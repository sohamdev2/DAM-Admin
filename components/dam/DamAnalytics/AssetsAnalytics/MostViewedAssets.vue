<template>
  <div class="tableDetail">
    <div class="common-box" :class="{ 'bg-gray': mostViewedAssetsList.length }">
      <div class="table-list-view table-list-scrolling">
        <ul v-if="mostViewedAssetsList.length" class="thead">
          <li>
            <div
              :class="[
        ...
        <div class="customscrollbar no_footer">
          <ContentLoader
            v-if="listLoading && !mostViewedAssetsList.length"
            :speed="1"
            :animate="true"
            :width="400"
            :height="100"
          ...
          <ul v-if="mostViewedAssetsList.length" class="tbody">
            <li v-for="file in mostViewedAssetsList" :key="file.id">
          ...
          <EmptyState v-if="!listLoading && !mostViewedAssetsList.length" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    files: {
      get() {
        return this.mostViewedAssetsList
      },
      set(value) {
        this.mostViewedAssetsList = value
      },
    },
  },
  watch: {
  ...
  methods: {
    ...
    async getMostViewedAssets() {
      this.listLoading = true
      await this.$axios
        .$post('digital-assets/analytics/most-view-assets', {
          from_date: this.startDate,
          to_date: this.endDate,
        })
        .then(({ data }) => {
          this.mostViewedAssetsList = data
