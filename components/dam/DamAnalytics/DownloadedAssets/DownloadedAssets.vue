  ...
  <div class="general-settings-right h-100">
    <div class="newAnalytics tableDetail customscrollbar">
      ...
      <div class="d-flex align-items-end justify-content-between mb2">
        <ul class="assetsType">
          <li v-if="chartType == 'assetsDownloadBySize'">
            <span>Size : </span>
            <div class="search-by">
              <Select2
          ...
          <li
            v-if="
              chartType == 'assetsDownloadByUser' ||
              chartType == 'assetsDownloadByDevice'
            "
          >
            <span>File Type : </span>
            <div class="search-by">
          ...
          <li v-if="chartType == 'assetsDownloadByUser'">
            <span>User Type : </span>
            <div class="search-by">
              <Select2
                class="select2-hidden-accessible"
          ...
          <li v-if="chartType == 'assetsDownloadByDevice'">
      ...
      <div class="common-box bg-gray">
        <div class="table-list-view table-list-scrolling">
          <ul v-if="downloadedAssetsList.length" class="thead">
            <li>
              <div
                :class="[
          ...
          <div class="customscrollbar no_footer">
            <ul class="tbody">
              <li v-for="file in downloadedAssetsList" :key="file.id">
              ...
              <client-only>
                ...
                >
                  ...
                  <div slot="no-results">
                    <EmptyState v-if="!downloadedAssetsList.length" />
                  </div>
                </infinite-loading>
              </client-only>
            </ul>
...
export default {
  ...
  computed: {
    categoryList() {
      ...
      switch (this.loadCategoryList) {
        case 'assetsDownloadBySize':
          x = this.bySizeAsset
          break
        case 'assetsDownloadByUser':
          x = this.userTypeList
          break
        case 'assetsDownloadByDevice':
          x = this.byDevice
          break
      }
      return x
    ...
    files: {
      ...
      set(value) {
        this.downloadedAssetsList = value
      },
    },
  },
  watch: {
  ...
  methods: {
    ...
    changeCategoryByFile(value) {
      ...
      this.resetDownloadedAssetsList()
    },
    ...
    resetDownloadedAssetsList() {
      this.downloadedAssetsList = []
      this.page = 1
      this.infiniteId += 1
    ...
    infiniteHandler($state) {
      ...
      if (this.chartType === 'assetsDownloadBySize') {
        this.getAssetsDownloadedBySize($state)
      }
      if (this.chartType === 'assetsDownloadByUser') {
        this.getAssetsDownloadedByUser($state)
      }
      if (this.chartType === 'assetsDownloadByDevice') {
        this.getAssetsDownloadedByDevice($state)
      }
    },
    async getAssetsDownloadedBySize($state) {
      let sizeType
      ...
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-download-by-size-details?page=${this.page}`,
          {
    ...
    async getAssetsDownloadedByUser($state) {
      ...
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-download-by-user-details?page=${this.page}`,
          {
            from_date: this.startDate,
            to_date: this.endDate,
            user_type: userType,
    ...
    async getAssetsDownloadedByDevice($state) {
      ...
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-download-by-device-details?page=${this.page}`,
          {
            from_date: this.startDate,
            to_date: this.endDate,
