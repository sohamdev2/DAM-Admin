  ...
  <div class="general-settings-right h-100">
    <div class="newAnalytics tableDetail customscrollbar">
      ...
      <div class="d-flex align-items-end justify-content-between mb2">
        <ul class="assetsType">
          <li v-if="chartType == 'assetsViewBySize'">
            <span>Size : </span>
            <div class="search-by">
              <Select2
          ...
          <li
            v-if="
              chartType == 'assetsViewByUser' ||
              chartType == 'assetsViewByDevice'
            "
          >
            <span>File Type : </span>
            <div class="search-by">
              <Select2
                class="select2-hidden-accessible"
                :value="categoryByFile"
                :options="byCategories"
                :attrs="{ minimumResultsForSearch: -1 }"
                @input="changeCategoryByFile"
              />
            </div>
          </li>
          <li v-if="chartType == 'assetsViewByUser'">
            <span>User Type : </span>
            <div class="search-by">
              <Select2
                class="select2-hidden-accessible"
          ...
          <li v-if="chartType == 'assetsViewByDevice'">
      ...
      <div class="common-box bg-gray">
        <div class="table-list-view table-list-scrolling">
          <ul v-if="viewedAssetsList.length" class="thead">
            <li>
              <div
                :class="[
              ...
              >
                <span @click="sortAssets('display_file_name')"
                  >Assets Name</span
                >
              </div>
              <div
              ...
              >
                <span @click="sortAssets('file_size')">Asset Size</span>
              </div>
              ...
              >
                <span @click="sortAssets('updated_at')">Last Modified</span>
              </div>
              <div
                :class="[
              ...
              >
                <span @click="sortAssets('view_count')">Total Views</span>
              </div>
            </li>
          </ul>
          <div class="customscrollbar no_footer">
            <ul class="tbody">
              <li v-for="file in viewedAssetsList" :key="file.id">
              ...
              <client-only>
                ...
                >
                  ...
                  <div slot="no-results">
                    <EmptyState v-if="!viewedAssetsList.length" />
                  </div>
                </infinite-loading>
              </client-only>
            </ul>
...
export default {
  ...
  computed: {
    categoryList() {
      ...
      switch (this.loadCategoryList) {
        case 'assetsViewBySize':
          x = this.bySizeAsset
          break
        case 'assetsViewByUser':
          x = this.userTypeList
          break
        case 'assetsViewByDevice':
          x = this.byDevice
          break
      }
      return x
    ...
    files: {
      get() {
        return this.viewedAssetsList
      },
      set(value) {
        this.viewedAssetsList = value
      },
    },
  },
  watch: {
  ...
  methods: {
    ...
    changeCategoryByFile(value) {
      this.categoryByFile = value
      this.resetViewedAssetsList()
    },
    ...
    resetViewedAssetsList() {
      this.viewedAssetsList = []
      this.page = 1
      this.infiniteId += 1
    ...
    infiniteHandler($state) {
      ...
      if (this.chartType === 'assetsViewBySize') {
        this.getAssetsViewedBySize($state)
      }
      if (this.chartType === 'assetsViewByUser') {
        this.getAssetsViewedByUser($state)
      }
      if (this.chartType === 'assetsViewByDevice') {
        this.getAssetsViewedByDevice($state)
      }
    },
    async getAssetsViewedBySize($state) {
      let sizeType
      if (this.categoryBy === '1 byte - 100 kb') {
        sizeType = '100kb_less_than'
      } else if (this.categoryBy === '101 kb - 1 mb') {
        sizeType = '101kb_to_1mb'
      } else if (this.categoryBy === '1 mb - 10 mb') {
        sizeType = '1mb_to_10mb'
      } else if (this.categoryBy === '11 mb - 100 mb') {
        sizeType = '11mb_to_100mb'
      } else if (this.categoryBy === '101 mb - 1 gb') {
        sizeType = '101mb_to_1gb'
      } else if (this.categoryBy === '1 gb') {
        sizeType = '1gb_to_grater_than'
      } else if (this.categoryBy === 'all') {
        sizeType = 'all'
      }
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-view-by-size-details?page=${this.page}`,
          {
            ...
            sort_value: this.sortValueModel,
          }
        )
        .then(({ data }) => {
          ...
          if (data.asset_data.data && data.asset_data.data.length) {
            ...
            if (
              ...
              this.viewedAssetsList.push(...data.asset_data.data)
              $state.loaded()
    ...
    async getAssetsViewedByUser($state) {
      let userType = this.categoryBy
      if (this.categoryBy === 'frontend users') {
        userType = 'frontend_user'
      }
      let fileType = this.categoryByFile
      if (this.categoryByFile === 'document') {
        fileType = 'application'
      }
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-view-by-user-details?page=${this.page}`,
          {
            from_date: this.startDate,
            to_date: this.endDate,
            user_type: userType,
    ...
    async getAssetsViewedByDevice($state) {
      let deviceType = this.categoryBy
      if (this.categoryBy === 'web') {
        deviceType = 'desktop'
      }
      let fileType = this.categoryByFile
      if (this.categoryByFile === 'document') {
      ...
      await this.$axios
        .$post(
          `digital-assets/analytics/assets-view-by-device-details?page=${this.page}`,
          {
            from_date: this.startDate,
            to_date: this.endDate,
    ...
    sortAssets(value) {
      this.sortValueModel = value
      this.sorting.toolbar.value = value
