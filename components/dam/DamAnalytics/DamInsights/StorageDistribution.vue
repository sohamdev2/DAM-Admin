  ...
  <div class="general-settings-right h-100">
    <div class="newAnalytics tableDetail customscrollbar">
      ...
      <div class="d-flex align-items-end justify-content-between mb2">
        <ul class="assetsType">
          <li v-if="chartType == 'storageDistributionByFile'">
            <span>File Type : </span>
            <div class="search-by">
              <Select2
          ...
          <li v-if="chartType == 'storageDistributionBySize'">
            <span>Size : </span>
            <div class="search-by">
              <Select2
                class="select2-hidden-accessible"
      ...
      <div class="common-box bg-gray">
        <div class="table-list-view table-list-scrolling">
          <ul v-if="storageDistributionList.length" class="thead">
            <li>
              <div
                :class="[
                  'categary-name sorting flex60',
          ...
          <div class="customscrollbar no_footer">
            <ul class="tbody">
              <li v-for="file in storageDistributionList" :key="file.id">
              ...
              <client-only>
                ...
                >
                  ...
                  <div slot="no-results">
                    <EmptyState v-if="!storageDistributionList.length" />
                  </div>
                </infinite-loading>
              </client-only>
            </ul>
...
export default {
  ...
  computed: {
    categoryList() {
      ...
      switch (this.loadCategoryList) {
        case 'storageDistributionByFile':
          x = this.groupByCategories
          break
        case 'storageDistributionBySize':
          x = this.bySizeAsset
          break
      }
      return x
    ...
    files: {
      get() {
        return this.storageDistributionList
      },
      set(value) {
        this.storageDistributionList = value
      },
    },
  },
  watch: {
  ...
  methods: {
    ...
    changeCategory(value) {
      ...
      this.resetStorageDistributionList()
    },
    assetsPermissionType(permission) {
      this.permissionBy = permission
      this.resetStorageDistributionList()
    },
    resetStorageDistributionList() {
      this.storageDistributionList = []
      this.page = 1
      this.infiniteId += 1
    ...
    infiniteHandler($state) {
      ...
      if (this.chartType === 'storageDistributionByFile') {
        this.getStorageDistributionByFileType($state)
      }
      if (this.chartType === 'storageDistributionBySize') {
        this.getStorageDistributionByFileSize($state)
      }
    },
    async getStorageDistributionByFileType($state) {
      let fileType = this.categoryBy
      if (this.categoryBy === 'document') {
        fileType = 'application'
      }
      await this.$axios
        .$post(
          `digital-assets/analytics/storage-distribution-by-file-type-details?page=${this.page}`,
          {
            file_type: fileType,
        ...
        .then(({ data }) => {
          ...
          if (data.asset_data.data && data.asset_data.data.length) {
            if (
              ...
              this.storageDistributionList.push(...data.asset_data.data)
              $state.loaded()
    ...
    async getStorageDistributionByFileSize($state) {
      let sizeType
      ...
      } else if (this.categoryBy === '1 gb' || this.categoryBy === '> 1 gb') {
      ...
      await this.$axios
        .$post(
          `digital-assets/analytics/storage-distribution-by-file-size-details?page=${this.page}`,
          {
            size_type: sizeType,
            permission: this.permissionBy,
          }
