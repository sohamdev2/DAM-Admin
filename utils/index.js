...
export function toHumanlySize(size, divider = 1024) {
...
export function toHumanlyNumericSize(size, divider = 1024) {
  const sizes = ['B', 'KB', 'MB', 'GB', 'TB', 'PB']
  size = Number(size)
  let index = 0
  ...
  const _label = parseInt(Math.ceil(size)) + ' ' + sizes[index]
  if (rollingLabel !== _label) {
    rollingLabel = _label
    return rollingLabel
  }

  // return parseInt(Math.ceil(size)) + ' ' + sizes[index]
}
...
export function getFormattedMetaValue(value, key, tooltip) {
  switch (key) {
    ...
    case 'FileSize':
      if (parseInt(value) && typeof value !== 'string')
